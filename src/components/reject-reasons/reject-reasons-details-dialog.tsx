...
// Read-only "view full record" dialog for a single Reject Reason. Mirrors
// CategoriesDetailsDialog's / LoadingPointsDetailsDialog's /
// DeliveryTypesDetailsDialog's structure: a header (code + Thai name +
// status badge), a Data Sheet with every field on the row, a description
// block (only when non-empty), a created/updated line, and a footer with
// Close (and Edit when the viewer is permitted). Same DialogContent
// `fullScreenOnMobile size="lg"` recipe — reject reasons is a small flat
...
// / Delivery Types' own `DataSheetRow`, kept local (consistent with this
// project's "hand-write each resource's own dialog components, don't
// extract one generic component" preference — see AGENTS.md § Materials PC).
...
export function RejectReasonsDetailsView({
  rejectReason,
  canEdit = false,
  onEdit,
  onClose,
  ...
  const nameEn = rejectReason.nameEn?.trim() || null;
  const description = rejectReason.description?.trim() || null;
  ...
  return (
    <div>
      {/* HERO — code eyebrow + Thai name + status badge. Mirrors
          ...
          DeliveryTypesDetailsView's hero structure so the four
          simple-master dialogs read the same way. */}
      <header className="border-b border-border px-6 py-6">
        <div className="flex flex-col gap-3">
          <p className="font-mono text-xs tracking-[0.05em] text-fg-muted">{rejectReason.code}</p>
      ...
      {/* DATA SHEET — every field on RejectReason. Same DataSheetRow +
          table shape as Categories' / Loading Points' / Delivery Types'
          details dialog so the four dialogs read the same way. Reject
          reason has the same number of fields as Loading Point and
          Delivery Type, so the Data Sheet is the same length. */}
      <section className="px-6 py-5">
        <SectionLabel>ข้อมูลทั่วไป</SectionLabel>
        <DataSheetTable>
          <DataSheetRow label="รหัส" value={rejectReason.code} />
          <DataSheetRow label="ชื่อ (ไทย)" value={rejectReason.nameTh} />
      ...
      <section className="px-6 pb-5">
        <p className="text-xs text-fg-muted">
          เพิ่มเมื่อ <span className="text-fg-secondary">{formatDateTime(rejectReason.createdAt)}</span>
          <span className="mx-2 text-border-strong">·</span>
          แก้ไขล่าสุด <span className="text-fg-secondary">{formatDateTime(rejectReason.updatedAt)}</span>
        </p>
      </section>
...
export function RejectReasonsDetailsDialog({
  rejectReason,
  canEdit = false,
  onEdit,
  onOpenChange,
