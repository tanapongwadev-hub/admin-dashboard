    ...
    <div className="flex flex-col items-center gap-4 sm:flex-row sm:items-center">
      <div className="relative shrink-0">
        ...
        <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
          <span className="text-lg font-semibold tabular-nums text-fg">{formatNumber(total)}</span>
          <span className="text-[11px] text-fg-muted">ผู้เข้าชม</span>
        </div>
      </div>
      <div className="flex flex-1 flex-col gap-2.5">
        {trafficSources.map((source) => (
          <div key={source.name} className="flex items-center justify-between gap-2 text-sm">
            ...
            <span className="tabular-nums font-medium text-fg">{formatNumber(source.value)}</span>
          </div>
        ))}
      </div>
    </div>
