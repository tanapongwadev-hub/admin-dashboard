...
test("all public Unit action wrappers forward to their perform* helpers", async (t) => {
  ...
  await assert.rejects(() => createUnitAction(makeCreatePayload()));
  await assert.rejects(() => updateUnitAction("unit-1", makeUpdatePayload()));
  await assert.rejects(() => deactivateUnitAction("unit-1"));
  await assert.rejects(() => restoreUnitAction("unit-1"));
