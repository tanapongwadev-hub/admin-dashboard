...
test("all public Category action wrappers forward to their perform* helpers", async (t) => {
  ...
  const createResult = await createCategoryAction(makeCreatePayload());
  assert.equal(createResult.status, "error");
  assert.match((createResult as { message: string }).message, /เซสชันของคุณหมดอายุ/);

  const updateResult = await updateCategoryAction("cat-1", makeUpdatePayload());
  assert.equal(updateResult.status, "error");

  const deactivateResult = await deactivateCategoryAction("cat-1");
  assert.equal(deactivateResult.status, "error");

  const restoreResult = await restoreCategoryAction("cat-1");
