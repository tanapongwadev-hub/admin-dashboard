...
test("all public DeliveryType action wrappers forward to their perform* helpers", async (t) => {
  ...
  const createResult = await createDeliveryTypeAction(makeCreatePayload());
  assert.equal(createResult.status, "error");
  assert.match((createResult as { message: string }).message, /เซสชันของคุณหมดอายุ/);

  const updateResult = await updateDeliveryTypeAction("dt-1", makeUpdatePayload());
  assert.equal(updateResult.status, "error");

  const deactivateResult = await deactivateDeliveryTypeAction("dt-1");
  assert.equal(deactivateResult.status, "error");

  const restoreResult = await restoreDeliveryTypeAction("dt-1");
