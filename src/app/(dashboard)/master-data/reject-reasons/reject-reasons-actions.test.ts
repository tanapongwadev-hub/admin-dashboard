...
test("all public RejectReason action wrappers forward to their perform* helpers", async (t) => {
  ...
  await assert.rejects(() => createRejectReasonAction(makeCreatePayload()));
  await assert.rejects(() => updateRejectReasonAction("rr-1", makeUpdatePayload()));
  await assert.rejects(() => deactivateRejectReasonAction("rr-1"));
  await assert.rejects(() => restoreRejectReasonAction("rr-1"));
